...
export async function loadInitialData() {
    try {
        ...
        appState.friends = Array.isArray(friends) ? friends.map(f => typeof f === "string" ? f : f.username).filter(Boolean) : [];
        ...
        appState.servers = Array.isArray(servers?.servers) ? servers.servers :
...
function warmProfileCache() {
    const toFetch = new Set([
        ...appState.friends,
        ...appState.dms.map(d => d.username)
    ]);
    toFetch.forEach((f) => {
        if (!f) return;
        if (!appState.profileCache[f]) {
            api.profile.getUser(f).then((p) => {
...
export async function fetchMessages(target) {
    try {
        ...
        const res = Array.isArray(msgs?.messages) ? msgs.messages :
        ...
        res.forEach(m => {
            ...
            if (sender && sender !== appState.username && !appState.profileCache[sender]) {
                api.profile.getUser(sender).then(u => {
