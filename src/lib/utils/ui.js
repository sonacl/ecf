...
export function initFontPreloader() {
    ...
    el.style.cssText = 'position:absolute;left:-9999px;top:-9999px;visibility:hidden;pointer-events:none;font-size:1px;';
    el.innerHTML =
...
export function parseMessageContent(content) {
    ...
    if (match) {
        ...
        try {
            ...
            try { replyPreview = decodeURIComponent(replyPreview); } catch (e2) { }
        }
        return {
...
export function groupMessages(messages) {
    ...
    const grouped = [];
    let currentGroup = { author: messages[0].author, timestamp: messages[0].timestamp, items: [messages[0]] };
