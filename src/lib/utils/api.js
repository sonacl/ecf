...
export const api = {
    ...
    profile: {
        ...
        changeUsername: (newUsername) => apiRequest('/enchatted/auth/change-username', 'POST', { new_username: newUsername }),
        changePassword: (oldPw, newPw) => apiRequest('/enchatted/auth/change-password', 'POST', { old_password: oldPw, new_password: newPw }),
        getCustomStatus: (username) => apiRequest(`/enchatted/profile/custom-status/${encodeURIComponent(username)}`),
        getAvatar: (username) => apiRequest(`/enchatted/profile/avatar/${encodeURIComponent(username)}`),
    ...
    messages: {
        ...
        getUpdates: (withUser, since) => apiRequest(`/enchatted/messages/updates?with=${encodeURIComponent(withUser)}&since=${encodeURIComponent(since)}`)
    },
    dms: {
    ...
    groups: {
        ...
        edit: (id, mid, content) => apiRequest(`/enchatted/groups/${id}/messages/edit`, 'POST', { id: mid, content }),
        react: (gid, mid, type) => apiRequest(`/enchatted/groups/${gid}/messages/react`, 'POST', { id: mid, type }),
    ...
    servers: {
        ...
        join: (inviteCode) => apiRequest('/enchatted/servers/join', 'POST', { invite_code: inviteCode }),
        ...
        createChannel: (id, name, description) => apiRequest(`/enchatted/servers/${id}/channels`, 'POST', { name, description }),
        ...
        getChannelMessages: (sid, cid) => apiRequest(`/enchatted/servers/${sid}/channels/${cid}/messages`),
        sendChannelMessage: (sid, cid, content) => apiRequest(`/enchatted/servers/${sid}/channels/${cid}/messages`, 'POST', { content }),
        react: (sid, cid, mid, type) => apiRequest(`/enchatted/servers/${sid}/channels/${cid}/messages/${mid}/react`, 'POST', { type }),
        edit: (sid, cid, mid, content) => apiRequest(`/enchatted/servers/${sid}/channels/${cid}/messages/${mid}/edit`, 'POST', { content }),
        delete: (sid, cid, mid) => apiRequest(`/enchatted/servers/${sid}/channels/${cid}/messages/${mid}/delete`, 'POST', {})
    },
    media: {
    ...
    admin: {
        ...
        updateUser: (username, data) => apiRequest(`/enchatted/api/users/${encodeURIComponent(username)}`, 'POST', data),
        resetPassword: (username, password) => apiRequest(`/enchatted/api/users/${encodeURIComponent(username)}/reset-password`, 'POST', { password }),
        enchant: (username, enchant, level = 1) => apiRequest(`/enchatted/api/users/${encodeURIComponent(username)}/enchant`, 'POST', { enchant, level }),
        removeEnchant: (username, enchant) => apiRequest(`/enchatted/api/users/${encodeURIComponent(username)}/remove-enchant`, 'POST', { enchant }),
        deactivate: (username) => apiRequest(`/enchatted/api/users/${encodeURIComponent(username)}/deactivate`, 'POST', {}),
        reactivate: (username) => apiRequest(`/enchatted/api/users/${encodeURIComponent(username)}/reactivate`, 'POST', {})
    },
    upload: (formData) => {
        if (!browser) return null;
        const headers = {};
