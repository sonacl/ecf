...
function handleNewMessage(msg) {
  ...
  // Auto-reopen DM in sidebar if it was closed
  const dmUser = m.sender === appState.username ? m.receiver : m.sender;
  if (dmUser && !m.channel_id) {
    if (!appState.dms.some((d) => d.username === dmUser)) {
      appState.dms = [{ username: dmUser, unread: 0 }, ...appState.dms];
    }
  }
  if (m.channel_id) {
