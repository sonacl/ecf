<script>
    ...
    $effect(() => {
        if (target && !isGroup && !target.startsWith("channel:")) {
            loading = true;
            Promise.all([
                api.profile.getUser(target),
                api.profile.getStats(target),
            ])
                .then(([u, s]) => {
                    userData = u;
                    stats = s;
    ...
    const enchantments = $derived.by(() => {
        if (!userData) return [];
        const items = [];
        if (userData.official) items.push("official");
        if (userData.owner) items.push("owner");
        const rawBadges = [
            ...(userData.badges || userData.enchantments || []),
            ...(stats?.badges || stats?.enchantments || []),
        ];
        rawBadges.forEach((b) => {
            if (typeof b === "string") items.push({ type: b, level: 0 });
            else if (b && typeof b === "object") {
                const type = b.enchant || b.type || b.name || b.id;
                if (type) items.push({ type, level: b.level || 0 });
            }
        });
        const unique = [];
        const seen = new Set();
