<script>
    ...
    import { refreshFriendData } from "$lib/utils/chat.js";
    ...
    const onlineFriends = $derived(
        appState.friends.filter((f) => appState.onlineUsers.has(f)),
    );
        ...
        return [
            {
                ...
                onClick: () => (appState.currentChat = friend),
            },
            {
                title: "Remove Friend",
                icon: "lucide:user-x",
